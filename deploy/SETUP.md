# Borg Deployment — app.example.com
...
1. [dash.cloudflare.com](https://dash.cloudflare.com) → **Add a Site** → `example.com` → Free plan
2. Copy the two nameservers Cloudflare assigns
3. At your registrar → `example.com` → replace nameservers with Cloudflare's
...
2. Type: **Self-hosted**, domain: `app.example.com`
...
# Clone repo
git clone https://github.com/borg-engine/borg.git /opt/borg
cd /opt/borg
...
Dashboard is now live at `https://app.example.com` behind Cloudflare Access.
