    ...
    pub async fn run_worker(
        ...
        loop {
            ...
            for message in messages {
                ...
                let embed_registry = Arc::clone(&embed_registry);
    ...
    if let Some(os) = search {
        ...
        if !chunks_text.is_empty() {
            ...
            let project_mode = db
                ...
                .flatten()
                .map(|p| p.mode);
            let ec = match project_mode.as_deref() {
                Some(mode) => embed_registry.client_for_mode(mode),
                None => embed_registry.default_client(),
            };
            let dim = ec.dim();
            let chunks_with_embeddings = batch_embed_chunks(&chunks_text, Some(ec), dim).await;
...
pub(crate) fn detect_doc_type(file_name: &str, mime: &str, text: &str) -> String {
    ...
    if mime.contains("pdf") || ext == "pdf" {
        ...
        if first_2k.contains("court")
            || first_2k.contains("plaintiff")
            || first_2k.contains("defendant")
            || first_2k.contains("v.")
        {
            return "filing".to_string();
        }
        if first_2k.contains("statute")
            || (first_2k.contains("section") && first_2k.contains("chapter"))
        {
...
pub(crate) fn detect_jurisdiction(text: &str) -> String {
    // Ordered by specificity: multi-word phrases first to avoid false positives
    static JURISDICTIONS: &[(&str, &str)] = &[
        // US states (most common in contracts)
    ...
    let first_4k = if text.len() > 4000 {
        &text[..text.floor_char_boundary(4000)]
    } else {
        text
    };
    for &(pattern, label) in JURISDICTIONS {
        if first_4k.contains(pattern) {
...
pub(crate) async fn extract_text_from_bytes(
    file_name: &str,
    mime: &str,
    bytes: &[u8],
) -> Result<String> {
