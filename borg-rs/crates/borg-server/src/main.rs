...
async fn main() -> anyhow::Result<()> {
    ...
    backends.insert(
        ...
        Arc::new(
            ClaudeBackend::new("claude", sandbox_mode.clone(), &config.container_image)
                ...
                .with_base_url(format!("http://127.0.0.1:3132")),
        ),
    );
    ...
    // Forward pipeline events to SSE log stream; route Notify events to transports
    {
        let log_tx_fwd = log_tx.clone();
        let tg_token_notify = config.telegram_token.clone();
        let chat_tx_fwd = chat_event_tx.clone();
        tokio::spawn(async move {
                            ...
                            if let Some(raw_id) = chat_id.strip_prefix("tg:") {
                                if !tg_token_notify.is_empty() {
                                    if let Ok(chat_id_i64) = raw_id.parse::<i64>() {
                                        let tg = borg_core::telegram::Telegram::new(
                                            tg_token_notify.clone(),
                                        );
                                        let _ = tg.send_message(chat_id_i64, message, None).await;
                                    }
                                }
                            } else {
                                // Web/project chat threads — broadcast as chat event
                                let event = json!({
                                    "role": "system",
                                    "sender": "pipeline",
                                    "text": message,
