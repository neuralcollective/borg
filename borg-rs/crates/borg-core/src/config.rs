...
fn get_csv(key: &str, dotenv: &HashMap<String, String>) -> Vec<String> {
    get(key, dotenv)
        .unwrap_or_default()
        .split(',')
        .map(str::trim)
    ...
    pub fn chat_system_prompt(&self) -> String {
        format!(
            "You are {name}, an autonomous AI agent collective. \
             Always refer to yourself as {name}. Keep replies concise.\n\n\
             ## Available Tools\n\
             ...
             Use `get_task_status` and `list_project_tasks` to track work. \
             If the task must review the full matter corpus before making corpus-wide conclusions, create it with `requires_exhaustive_corpus_review=true`. \
             Ask clarifying questions first if the request is ambiguous.\n\n\
             ## External Research\n\
             If legal research tools are available (lawborg-mcp), use them for case law, statutes, \
             filings, and regulatory lookups. Always prefer structured API tools over web search \
             when a relevant tool exists. Check `list_services` output to know what's available.",
