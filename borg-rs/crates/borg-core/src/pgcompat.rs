...
impl ConnectionGuard {
    ...
    fn with_client<T>(&self, f: impl FnOnce(&deadpool_postgres::Client) -> Result<T>) -> Result<T> {
        match &self.state {
            ...
            GuardState::Ready(None) => Err(Error::SessionUnavailable(
                "postgres session already released".into(),
            )),
...
impl<'a> Statement<'a> {
    pub fn query_map<P: ParamsLike, T, F>(
        &mut self,
        params: P,
        mut mapper: F,
    ) -> Result<MappedRows<T>>
        ...
        F: FnMut(&Row<'_>) -> Result<T>,
    {
        ...
        let rows = self
            .guard
            .with_client(|client| Ok(block_on(client.query(&translated, &refs))?))?;
        let mapped = rows
            .into_iter()
            .map(|row| {
                let row = Row::new(row);
