...
impl Telegram {
    ...
    pub async fn connect(&mut self) -> Result<()> {
        ...
        if let Some(username) = resp["result"]["username"].as_str() {
            ...
            self.bot_user_id = resp["result"]["id"].as_i64().unwrap_or(0);
    ...
    pub async fn get_updates(&self) -> Result<Vec<TgMessage>> {
        ...
        for update in updates {
            ...
            let chat = msg.get("chat").unwrap_or(&Value::Null);
            ...
            let chat_title = chat["title"]
                ...
                .unwrap_or("")
                .to_string();
            let sender_id = from["id"].as_i64().unwrap_or(0);
            ...
            let reply_to_bot = reply
                .get("from")
                .and_then(|r| r["id"].as_i64())
                .map(|id| id == self.bot_user_id && self.bot_user_id != 0)
                .unwrap_or(false);

            let reply_to_text = reply
                .get("text")
                .and_then(Value::as_str)
                .map(|s| s.to_string());
            let reply_to_author = reply
                .get("from")
                .and_then(|r| r["first_name"].as_str())
                .map(|s| s.to_string());

            messages.push(TgMessage {
                message_id: msg.get("message_id").and_then(Value::as_i64).unwrap_or(0),
                chat_id,
                chat_type,
                chat_title,
                sender_id,
    ...
    pub async fn send_message(
        ...
        self.send_message_internal(chat_id, text, reply_to, Some("Markdown"))
            .await
    }
    ...
    pub async fn send_plain_message(
        ...
        self.send_message_internal(chat_id, text, reply_to, None)
            .await
    }
    ...
    async fn send_message_internal(
        ...
        for chunk in split_text(text, 4000) {
            ...
            if parse_mode.is_some() {
                warn!(
                    "Telegram sendMessage failed with parse mode {:?}: {:?}; retrying without parse mode",
                    parse_mode,
                    resp["description"]
                );
                body.as_object_mut().map(|obj| obj.remove("parse_mode"));
                let plain_resp: Value = self
                    .client
                    .post(self.api_url("sendMessage"))
                    .json(&body)
                    .send()
                    ...
                    .json()
                    .await?;
                if plain_resp["ok"] != true {
                    warn!(
                        "Telegram sendMessage plain retry failed: {:?}",
                        plain_resp["description"]
                    );
                }
            } else {
...
mod tests {
    ...
    use super::{check_mentions_bot, split_text, Telegram};

    #[test]
    fn group_reply_to_bot_sets_reply_flag() {
        let mut tg = Telegram::new("test");
        tg.bot_user_id = 123;
        tg.bot_username = "borgTGBot".to_string();
        let update = json!({
            "message_id": 99,
            ...
            "reply_to_message": {
                ...
                "from": { "id": 123, "first_name": "Borg" }
            }
        });
        let entities: &[serde_json::Value] = update
            .get("entities")
            .and_then(|e| e.as_array())
            .map(|v| v.as_slice())
            .unwrap_or(&[]);
        let mentions_bot = check_mentions_bot(
            &tg.bot_username,
            update["text"].as_str().unwrap_or(""),
            entities,
        );
        let reply_to_bot = update
            .get("reply_to_message")
            .and_then(|r| r["from"]["id"].as_i64())
            .map(|id| id == tg.bot_user_id && tg.bot_user_id != 0)
            .unwrap_or(false);
        assert!(!mentions_bot);
        assert!(reply_to_bot);
    }
