...
impl Db {
    ...
    pub fn get_task_revision_count(&self, id: i64) -> Result<i64> {
        let conn = self.conn.lock().map_err(|_| anyhow::anyhow!("db mutex poisoned"))?;
        Ok(conn
            .query_row(
    ...
    pub fn embedding_count(&self) -> Result<i64> {
        let conn = self.conn.lock().map_err(|_| anyhow::anyhow!("db mutex poisoned"))?;
        Ok(conn
            .query_row("SELECT COUNT(*) FROM embeddings", [], |r: &rusqlite::Row| r.get(0))
            .unwrap_or(0))
    }
    ...
    pub fn count_unscored_proposals(&self) -> Result<i64> {
        let conn = self.conn.lock().map_err(|_| anyhow::anyhow!("db mutex poisoned"))?;
        Ok(conn
            .query_row(
            ...
            .unwrap_or_else(|e| {
                tracing::warn!("count_unscored_proposals: {e}");
                0
            }))
    }
    ...
    pub fn get_unknown_retries(&self, id: i64) -> Result<i64> {
        let conn = self.conn.lock().map_err(|_| anyhow::anyhow!("db mutex poisoned"))?;
        Ok(conn
            .query_row(
    ...
    pub fn active_task_count(&self) -> Result<i64> {
        ...
        Ok(conn
            ...
            .unwrap_or_else(|e| {
                tracing::warn!("active_task_count: {e}");
                0
            }))
    }
