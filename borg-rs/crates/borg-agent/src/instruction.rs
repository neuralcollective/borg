...
fn build_completion_gate_section(task: &Task, phase: &PhaseConfig, ctx: &PhaseContext) -> String {
    let verdict_schema = json!({
        ...
        "rationale": "Briefly explain what you checked against the original request.",
    ...
    format!(
        "## Completion Gate\n\
Original request reference for this run:\n\
- Task title: {title}\n\
...
\n\
Before you stop working on this phase, rigorously compare your current output against every active instruction source:\n\
- the original request reference above\n\
- the current phase instruction\n\
- chat context and queued user/director messages\n\
- prior reviewer feedback and prior failure context, if present\n\
\n\
Do not stop just because you made progress. Only finish when every explicit requirement that is actionable in this phase has been satisfied and checked.\n\
\n\
Before ending the phase, write `.borg/phase-verdict.json` with this exact JSON shape and exact metadata values for this run:\n\
```json\n\
{verdict_schema}\n\
```\n\
\n\
...
- Set `ready_to_advance` to `true` only when the request has been fulfilled for this phase and your work is ready for the next pipeline step.\n\
- `rationale` must briefly explain what you verified against the original request.\n\
- If `ready_to_advance` is `true`, `missing_requirements` must be empty.\n\
...
pub fn build_instruction(
    ...
    if task.requires_exhaustive_corpus_review {
        s.push_str(
            "## Review Scope\n\n\
             This task is explicitly marked as requiring exhaustive corpus review. \
             Do not make corpus-wide present/absent conclusions from spot retrieval alone. \
             You must inventory the attached corpus, iterate BorgSearch queries, run coverage checks, \
             and inspect full documents before claiming exhaustive findings.\n\n---\n\n",
        );
    }

    if !ctx.chat_context.is_empty() {
        s.push_str("## Originating Conversation\n\nThis task was created from a chat conversation. Here is the relevant discussion for context:\n\n");
        for (sender, content) in &ctx.chat_context {
            s.push_str(&format!("**{}**: {}\n\n", sender, content));
        }
        s.push_str("---\n\n");
    }
