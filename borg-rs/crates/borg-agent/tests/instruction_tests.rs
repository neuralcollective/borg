...
fn make_ctx() -> PhaseContext {
    PhaseContext {
        ...
        phase_attempt: 1,
        phase_gate_token: "gate-token-123".to_string(),
...
#[test]
fn test_completion_gate_included_in_instruction() {
    ...
    assert!(
        result.contains("## Completion Gate"),
        "completion gate section must be present: {result}"
    );
    assert!(
        result.contains(".borg/phase-verdict.json"),
        "completion gate must require phase-verdict.json: {result}"
    );
    assert!(
        result.contains("\"task_id\": 1"),
        "completion gate must bind task_id metadata: {result}"
    );
    assert!(
        result.contains("\"attempt\": 1"),
        "completion gate must bind attempt metadata: {result}"
    );
    assert!(
        result.contains("\"gate_token\": \"gate-token-123\""),
        "completion gate must bind gate token metadata: {result}"
    );
    assert!(
        result.contains("\"ready_to_advance\": true"),
        "completion gate must show verdict schema: {result}"
    );
}
...
fn test_include_task_context_false_omits_title_and_description() {
    ...
    assert!(
        ...
        "main task context block should be absent when include_task_context=false: {result}"
    );
    assert!(
        result.contains("- Task title: My Task Title"),
        "completion gate should still carry the original request reference: {result}"
    );
}
