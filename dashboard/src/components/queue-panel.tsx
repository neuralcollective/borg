  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-14 shrink-0 items-center justify-between border-b border-[#2a2520] px-5">
        <span className="text-[14px] font-semibold text-[#e8e0d4]">Integration Queue</span>
        <span className="rounded-full bg-amber-500/[0.06] px-2.5 py-0.5 text-[11px] tabular-nums text-[#9c9486]">{filtered?.length ?? 0}</span>
      </div>
      <div className="flex-1 overflow-y-auto overscroll-contain">
        <div className="p-2">
          {filtered?.map((e) => (
            <div key={e.id} className="flex items-center gap-3 rounded-xl px-4 py-3 text-[13px] transition-colors hover:bg-[#1c1a17]">
              <StatusBadge status={e.status} />
              {multiRepo && !repoFilter && e.repo_path && (
                <span className="shrink-0 rounded-lg bg-amber-500/[0.04] px-2 py-0.5 text-[11px] font-medium text-[#9c9486]">
          ...
          {!filtered?.length && (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-amber-500/[0.04] ring-1 ring-amber-900/20">
                <GitMerge className="h-6 w-6 text-[#6b6459]" strokeWidth={1.5} />
              </div>
              <p className="text-[14px] text-[#9c9486]">Queue is empty</p>
              <p className="mt-1 text-[12px] text-[#6b6459]">Completed tasks will appear here for integration</p>
            </div>
          )}
        </div>
      </div>
