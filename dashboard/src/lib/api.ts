...
export async function verifyTaskCitations(
  id: number,
): Promise<{ verified: number; total: number; citations: CitationVerification[] }> {
...
export async function fetchProjectFileContent(projectId: number, fileId: number): Promise<ArrayBuffer> {
...
export async function uploadProjectFiles(
  ...
  const res = await fetch(`${apiBase()}/api/projects/${projectId}/files/upload${query ? `?${query}` : ""}`, {
    method: "POST",
    headers: authHeaders(),
    body: form,
  });
...
export async function uploadProjectUploadChunk(
  ...
  await tokenReady;
  const res = await fetch(`${apiBase()}/api/projects/${projectId}/uploads/sessions/${sessionId}/chunks/${chunkIndex}`, {
    method: "PUT",
    headers: authHeaders(),
    body: chunk,
  });
...
export async function summarizeWorkspaceThemes(opts: { limit?: number; minDocs?: number } = {}): Promise<ThemeSummary> {
  const params = new URLSearchParams();
  if (opts.limit) params.set("limit", String(opts.limit));
  if (opts.minDocs) params.set("min_docs", String(opts.minDocs));
  const qs = params.toString();
...
export function useProjectDocumentVersions(projectId: number | null, taskId: number | null, path: string | null) {
  return useQuery<{ sha: string; message: string; date: string; author: string }[]>({
    ...
    queryFn: () =>
      fetchJson(`/api/projects/${projectId}/documents/${taskId}/versions?path=${encodeURIComponent(path!)}`),
...
export async function sendProjectChat(projectId: number, text: string, sender = "web-user"): Promise<{ ok: boolean }> {
...
export async function deleteAllKnowledgeFiles(): Promise<{ ok: boolean; deleted: number }> {
